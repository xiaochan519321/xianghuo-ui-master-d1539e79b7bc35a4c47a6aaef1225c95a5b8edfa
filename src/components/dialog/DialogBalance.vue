    ...
    <el-dialog
        ...
        <el-form :model="formData" ref="form" label-width="90px" label-position="left">
            ...
            <el-form-item label="提现佣金" prop="money">
                <el-input v-model="formData.money" placeholder="请输入提现佣金"></el-input>
            </el-form-item>
...
import { applyWithdrawal } from '@/api/admin/ddkOrder'
export default {
    components: {},
    data() {
        return {
    ...
    methods: {
        ...
        async onSubmit () {
            ...
            try {
                const { data } = await applyWithdrawal(params)
                this.loading = false
                if (data.code == 0) {
                    ...
                    // 更新站点信息
                    this.$store.dispatch('GetSiteConfig', { type: true })
                    this.$emit('success')
                }
            } catch (err) {
                this.loading = false
