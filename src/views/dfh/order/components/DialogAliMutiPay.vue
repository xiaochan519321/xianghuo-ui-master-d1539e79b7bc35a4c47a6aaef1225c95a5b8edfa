    ...
    <div class="order-muti-pay-dialog">
        <el-dialog
            ...
            <div class="order-pay-content" v-loading="loading">
                ...
                <div class="img-box text-center" v-if="payUrl">
                    <qrcode :value="payUrl" :options="options" />
                </div>
                <el-empty v-else description="获取支付二维码失败，请刷新重试" :image-size="150"></el-empty>
                <div class="tips" v-if="payUrl">提示：请勿关闭页面，支付成功后自动检测支付状态，仅支持支付宝扫码付款</div>
            </div>
        </el-dialog>
    </div>
</template>
...
import { create1688pay, get1688OrderDetails, get1688PayStatus } from '@/api/dfh/order'
export default {
    components: {
        qrcode: VueQrcode
    },
    ...
    methods: {
        ...
        async onBalance (ids) {
            ...
            await Promise.all(arr).then(res => {
                ...
                if (res[0]) {
                    const { data } = res[0]
                    console.log('create1688pay', data)
                    this.payUrl = data.data || ''
                    // this.payData = data
                    this.startTimer()
                }
                // 批量订单详情
        ...
        async getPayOrderStatus() {
            const { data } = await get1688PayStatus({payUrl: this.payUrl})
