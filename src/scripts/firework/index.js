...
const store = {
	...
	state: {
		...
		config: {
			quality: '3', // will be mirrored to a global variable named `quality` in `configDidUpdate`, for perf.
			shell: 'Strobe',
			size: IS_DESKTOP
				? '3' // Desktop default
				: IS_HEADER 
...
function handlePointerStart(event) {
    if (event.config) {
        store.setState({
            config: Object.assign({}, store.state.config, event.config)
        })
        configDidUpdate()
    }
...
function render(speed) {
	...
	// trailsCtx.fillStyle = `rgba(0, 0, 0, 0.0)`;
	// trailsCtx.fillStyle = '#1d2642'
